...
function MaterialsModule({ apiBase, token, user }) {
  ...
  const handleSaveCatalog = async (id) => {
    if (isReadOnly) return;
    try {
      const res = await fetch(`${apiBase}/materials/${id}`, {
        method: 'PUT',
  ...
  const handleBookPriceChange = (code, val) => {
    setEditedBookPrices(prev => ({
      ...prev,
  ...
  const filteredBookItems = bookItems.filter(m => {
    const matchesCategory = filterCategory === 'all' || m.category === filterCategory;
    const matchesSearch = 
      m.code.toLowerCase().includes(searchQuery.toLowerCase()) || 
      m.name.toLowerCase().includes(searchQuery.toLowerCase());
