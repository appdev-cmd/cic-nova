...
function ProjectsModule({ apiBase, token, user, currentView, setCurrentView }) {
  ...
  const fetchOperaMaterials = async (projectId) => {
    try {
  ...
  const handleEditDoorSubmit = async (e) => {
    ...
    try {
      // 1. Save dimensions
      const resDims = await fetch(`${apiBase}/projects/${activeProject.id}/doors/${editForm.id}`, {
        method: 'PUT',
        headers: { 
          'Content-Type': 'application/json',
          ...(token ? { 'Authorization': `Bearer ${token}` } : {})
      ...
      if (!resDims.ok) {
        const err = await resDims.json();
        alert(`Lỗi khi lưu thông số cửa: ${err.detail || 'Lỗi không xác định'}`);
        return;
      }

      // 2. Save description & overrides
      const resOverrides = await fetch(`${apiBase}/projects/${activeProject.id}/doors/${editForm.id}/overrides`, {
        method: 'PUT',
        headers: { 
          'Content-Type': 'application/json',
          ...(token ? { 'Authorization': `Bearer ${token}` } : {})
      ...
      if (resOverrides.ok) {
        alert("Cập nhật thông tin cửa và định mức đè thành công!");
        setIsPanelOpen(false);
        fetchProjectDoors(activeProject.id);
        setCalcResults(null);
      } else {
        const err = await resOverrides.json();
        alert(`Lỗi khi lưu chi phí đè: ${err.detail || 'Lỗi không xác định'}`);
      }
    } catch (e) {
      console.error("Error updating door:", e);
      alert("Lỗi kết nối máy chủ khi cập nhật cửa.");
    }
  };
  ...
  const handleUpdateProject = async (event) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(await readApiError(response, 'Không thể cập nhật dự án.'));
      const updated = await response.json();
      const mergedProject = { ...editingProject, ...updated };
      setProjects((current) => current.map((project) => project.id === updated.id ? mergedProject : project));
      if (activeProject?.id === updated.id) setActiveProject(mergedProject);
      setShowEditProjectModal(false);
      setEditingProject(null);
    } catch (error) {
      alert(error instanceof Error ? error.message : 'Không thể cập nhật dự án.');
    }
  };
  ...
  const handleExportSplit = async () => {
    if (!activeProject || isExportingSplit) return;
    setIsExportingSplit(true);
    try {
      const response = await fetch(`${apiBase}/projects/${activeProject.id}/export-split`);
      if (!response.ok) {
        throw new Error(await readApiError(response, 'Không thể xuất báo cáo tách file.'));
      }
      await downloadResponse(response, `BAO_GIA_DU_AN_${activeProject.id}_SPLIT.zip`);
    } catch (error) {
      alert(error instanceof Error ? error.message : 'Không thể xuất báo cáo tách file.');
    } finally {
        ...
        <div>
          ...
          <div className="glass-panel" style={{ padding: '20px', marginBottom: '24px', display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button
                className="btn btn-secondary"
                title="Xuất 2 file riêng: Tổng hợp chi phí và Báo giá"
                style={{ padding: '8px 16px', fontSize: '13px' }}
                onClick={handleExportSplit}
                disabled={projectDoors.length === 0 || isExportingSplit}
              >
                <FileSpreadsheet size={14} style={{ marginRight: '6px' }} /> {isExportingSplit ? 'Đang xuất...' : 'Xuất 2 File Đính Kèm'}
              </button>
            </div>
          </div>
